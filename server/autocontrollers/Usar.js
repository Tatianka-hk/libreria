const User = require( '../models/usar')
const jwt = require('jsonwebtoken')
const Book = require('../models/book');
exports.add_to_favorites= (req, res)=>{
    try{
        console.log("add to fav")
        console.log(req.user.id)
        console.log(req.body.id)
        let b={book_id:req.body.id}
        User.findById(req.user.id).then((data)=>{console.log(data)})
        User
        .findByIdAndUpdate(req.user.id, {$push: {favorite:b}})
        .then((data)=>console.log("edit"))
        .catch((err)=>console.log(err))


    }catch(err){ console.log(err)}
}

exports.add_want_to_read= (req, res)=>{
    try{
        console.log("add to fav")
        console.log(req.user.id)
        console.log(req.body.id)
        let b={book_id:req.body.id}
        User.findById(req.user.id).then((data)=>{console.log(data)})
        User
        .findByIdAndUpdate(req.user.id, {$push: {want_to_read:b}})
        .then((data)=>console.log("edit"))
        .catch((err)=>console.log(err))


    }catch(err){ console.log(err)}
}

exports.see_fav= (req, res)=>{
    try{
        console.log("SEE FAV")
        User
        .findById(req.user.id)
        .then((user)=>{
            console.log("user" - user)
            let fav_list = user.favorite;
            // console.log(fav_list)
            Book
                .find()
                .then((books)=>{
                    let list=[];
                    books.forEach(book=>{
                        console.log(book._id)
                        fav_list.forEach(fav_point=>{
                            
                            if (fav_point.book_id == book._id){
                                let point={
                                    id:book._id,
                                    name:book.name, 
                                    author: book.author,
                                    genre: book.genre,
                                    short_description: book.short_description,
                                    long_description: book.long_description
                                }
                                list.push(point)
                                console.log("si")
                            }
                        })
                        
                        
                    })
                    // console.log(list)
                    res.send({books:list})
                })
                .catch((err)=>console.log(err))

        })
        .catch((err)=>console.log(err))



    }catch(err){ console.log(err)}
}

exports.see_want_to_reed= (req, res)=>{
    try{
        console.log("SEE FAV")
        console.log(req.user.id)
        User
        .findById(req.user.id)
        .then((user)=>{
            console.log("user" - user)
            let fav_list = user.want_to_read;
            // console.log(fav_list)
            Book
                .find()
                .then((books)=>{
                    let list=[];
                    books.forEach(book=>{
                        console.log(book._id)
                        fav_list.forEach(fav_point=>{
                            
                            if (fav_point.book_id == book._id){
                                let point={
                                    id:book._id,
                                    name:book.name, 
                                    author: book.author,
                                    genre: book.genre,
                                    short_description: book.short_description,
                                    long_description: book.long_description
                                }
                                list.push(point)
                                console.log("si")
                            }
                        })
                        
                        
                    })
                    // console.log(list)
                    res.send({books:list})
                })
                .catch((err)=>console.log(err))

        })
        .catch((err)=>console.log(err))



    }catch(err){ console.log(err)}
}

exports.delete_from_favorites= (req, res)=>{
    try{

        console.log("delete to fav")
        console.log(req.user.id)
        console.log(req.body.id)
        let b={book_id:req.body.id}
        User
        .findById(req.user.id)
        .then((data)=>{
            console.log(data.login)
            let list=[];
            data.favorite.forEach(point=>{
                // console.log(point)
                // console.log(b)
                if (point.book_id != b.book_id){
                    list.push(point)
                    // console.log("si")
                }
                else{ 
                    // console.log("no")
                }
            })
            User
            .findByIdAndUpdate(req.user.id, {favorite:list})
            .then((data)=>console.log("deleted"))
            .catch((err)=>console.log(err))
        })
        .then((data)=>console.log("deleted"))
        .catch((err)=>console.log(err))


    }catch(err){ console.log(err)}
}


exports.delete_from_want_to_read= (req, res)=>{
    try{

        console.log("delete to fav")
        console.log(req.user.id)
        console.log(req.body.id)
        let b={book_id:req.body.id}
        User
        .findById(req.user.id)
        .then((data)=>{
            console.log(data.login)
            let list=[];
            data.want_to_read.forEach(point=>{
                // console.log(point)
                // console.log(b)
                if (point.book_id != b.book_id){
                    list.push(point)
                    // console.log("si")
                }
                else{ 
                    // console.log("no")
                }
            })
            User
            .findByIdAndUpdate(req.user.id, {want_to_read:list})
            .then((data)=>console.log("deleted"))
            .catch((err)=>console.log(err))
        })
        .then((data)=>console.log("deleted"))
        .catch((err)=>console.log(err))


    }catch(err){ console.log(err)}
}